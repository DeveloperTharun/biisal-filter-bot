...
BOT_TOKEN = environ.get('BOT_TOKEN', "6694217037:AAGIL5l-yNoRiOtvA0tZe-VwBjVz0s8GxR4") #if you are going to bot token here make sure your repo is private..
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001512818854').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL')
auth_grp = environ.get('AUTH_GROUP', '-1002087413701')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002087413701')
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002087413701')
...
SHORTLINK_API = environ.get('SHORTLINK_API', 'e36f2573636d8a5dd3b81b64ce434db80002867a') #use my api so i can make some money 🤣..just kidding guys..mai already aamir hoon🥱 
SECOND_SHORTLINK_URL = environ.get('SECOND_SHORTLINK_URL', 'ModiJiUrl.com')
SECOND_SHORTLINK_API = environ.get('SECOND_SHORTLINK_API', '8b2aa79be0fe87e9299264924b6909117f2c0f22')
...
VERIFY_REQ_CHNL = int(environ.get('VERIFY_REQ_CHNL', '-1002116593827'))
NEW_USER_LOG = int(environ.get('NEW_USER_LOG', '-1002116593827'))
BIN_CHNL = int(environ.get('BIN_CHNL', '-1002116593827'))
